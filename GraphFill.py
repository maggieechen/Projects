...
class ImageGraph:
    ...
    #input: node_index is the index of the current node
    #output is the list of non-visited edges of this node
    def not_visited_edges_helper(self,node_index):
        not_visited = []
        edges = self.nodes[node_index].edges 
        for i in range(len(edges)):
            if self.nodes[edges[i]].visited is not True:
                not_visited.append(edges[i])
        return not_visited
    ...
    def print_adjacency_matrix(self):
        ...
        for i in range(length):
            for j in range(len(self.nodes[i].edges)):
                matrix[i][self.nodes[i].edges[j]] = 1
        for i in range(length):
    ...
    def bfs(self, start_index, color):
        ...
        while not queue.is_empty():
            new = queue.dequeue()
            edge = self.nodes[new].edges
            for i in range(len(edge)):
                if self.nodes[edge[i]].visited is not True and self.nodes[edge[i]].color == self.nodes[start_index].prev_color:
                    self.nodes[edge[i]].visit_and_set_color(color)
    ...
    def dfs(self, start_index, color):
        ...
        while not stack.is_empty():
            new = stack.pop()
            if self.nodes[new].visited is not True and self.nodes[new].color == self.nodes[start_index].prev_color:
                self.nodes[new].visit_and_set_color(color)
                self.print_image()
                not_visited = self.not_visited_edges_helper(new)
                for i in reversed(range(len(not_visited))):
                    stack.push(not_visited[i])
